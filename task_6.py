# Реализовать два небольших скрипта:
# а) итератор, генерирующий целые числа, начиная с указанного,
# б) итератор, повторяющий элементы некоторого списка, определенного заранее.
# Подсказка: использовать функцию count() и cycle() модуля itertools. Обратите внимание, что создаваемый цикл не должен
# быть бесконечным. Необходимо предусмотреть условие его завершения.
# Например, в первом задании выводим целые числа, начиная с 3, а при достижении числа 10 завершаем цикл. Во втором также
# необходимо предусмотреть условие, при котором повторение элементов списка будет прекращено.

from itertools import count, cycle
from random import random

while 1:
    try:
        a = int(input("Введите число с которого начинать генератор: "))
        b = int(input("Введите число где закончить генератор: "))
        d = int(input("Введите число сколько прокрутить список случайных цисел: "))
        e = int(input("Введите число длину списка случайных цисел: "))
        break
    except:
        print("Проверте правильность введенных вами данных, вводить нужно числа")

for el in count(a):
    if el > b:
        break
    else:
        print(el)
с=0
for el in cycle([int(random()*100) for x in range(e)]):
    if с > d:
        break
    print(el)
    с += 1
